...
# Nous utiliserons cette fonction pour afficher des images avec Matplotlib
...
def _extract_face(filepath, face_cascade):
    ...
    if len(face) != 1:
        sys.exit(f"Exemple {filepath} n'a pas exactement un visage!")
...
def load_data(face_cascade, data_dir='yalefaces'):
    ...
    # Montrons les premières images de visage (à des fins de contrôle)
    for i in range(5):
        if i < len(X_train):
